...
<template>
  <div class="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    ...
    <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-lg">
      ...
      <TransitionRoot as="template" :show="expired_link_error">
        <Dialog as="div" static class="fixed z-10 inset-0 overflow-y-auto" @close="expired_link_error = false" :open="expired_link_error">
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                      <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900"> Expired link </DialogTitle>
                      <div class="mt-2">
                        <p class="text-sm text-gray-500">The link you are using has expired, please try to reset you password again</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                  <button type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" @click="expired_link_error = false" ref="cancelButtonRef">Cancel</button>
                </div>
              </div>
            </TransitionChild>
          </div>
  ...
  methods: {
    onSubmit(values) {
      ...
      if (values.password && values['re-password']) {
        AuthService.resetPassword(this.uid, this.token, values.password, values['re-password'])
          .then(() => {
