...
gnome-terminal -t "{{ cookiecutter.project_name }} Celery" -- pipenv run celery worker --app={{ cookiecutter.project_slug }}.celery_app --loglevel=info --workdir=server
{% endif %}
{% if cookiecutter.async.lower() == "async" %}
gunicorn -b $CURRENT_DOMAIN:$CURRENT_PORT --chdir=server {{ cookiecutter.project_slug }}.asgi:application -k uvicorn.workers.UvicornWorker
